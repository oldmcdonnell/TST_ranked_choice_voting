...
function timingSafeEqualStrings(a, b) {
  ...
  // on a length check alone, so comparison time doesn't leak length either.
  if (bufA.length !== bufB.length) {
    crypto.timingSafeEqual(bufA, bufA); // burn roughly equivalent time
    return false;
  }
...
export function requireAdminStub(req, reply, done) {
  ...
  const expected = process.env.ADMIN_STUB_KEY;
